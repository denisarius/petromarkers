...
// ---------------------------------------------------------------------------------------
var common_attachment_swfu=0, common_attachment_uploaded_files=new Array();
//------------------------------------------------------------------------------
function common_attachment_edit(attachmet_id, file, name, func)
{
	if (common_attachment_swfu)
	{
		common_attachment_swfu.destroy();
//		delete common_attachment_swfu;
	}

	admin_info_show('Прикрепленный документ', '<div style="text-align:center;"><img src="images/'+loading_icon+'"></div>', 440);
	html=execQuery(common_widget_ajax_manager_url, {section : "commonGetAttachmentEditHtml", attachmet_id: attachmet_id, file: file, name: name, func: func});
...
function common_attachment_save(func)
{
	...
	if (name=='')
	{
		show_message_warning("Сохранение документа", "Необходимо ввести название документа");
		return;
	}
