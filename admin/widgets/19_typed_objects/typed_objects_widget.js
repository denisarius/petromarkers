...
{
	menu_item=$("#typed_objects_menu_item_id").val();
	admin_info_show('Сортировка объектов', '<div style="text-align: center"><img src="images/'+loading_icon+'"></div>');
...
//------------------------------------------------------------------------------
var html_editor=0;
function typed_objects_html_edit(id)
{
	if (html_editor!=0)
	{
		CKEDITOR.remove(html_editor);
		html_editor.destroy(true);
		html_editor=0;
	}
	name=$('#name_'+id).val();
	html='<textarea id="type_objects_html_editor_container"></textarea>\
<hr>\
<input type="button" value="Записать" onClick="typed_objects_html_edit_save(\''+id+'\')"/>\
<input type="button" value="Отмена" onClick="typed_objects_html_edit_cancel()"/>\
';
	admin_info_show(name, "<img src='/admin/images/"+loading_icon+"'>", 800);
	prop_html=$('#prop_'+id).val();
	res=execQuery(typed_objects_widget_ajax_manager_url, {section : "typed_objectsHtmlPropGetHTML", html: prop_html});

	admin_info_change('', html);
	...
	CKEDITOR.config.format_tags = 'p';
	CKEDITOR.config.baseFloatZIndex=100100;
	html_editor=CKEDITOR.replace('type_objects_html_editor_container',
		{
		    on : { instanceReady : function( ev ) { this.focus(); } }
		});
//	html_editor.setData($('#prop_'+id).val());
	html_editor.setData(res);
	html_editor.focus();
}
//------------------------------------------------------------------------------
function typed_objects_html_edit_cancel()
{
	if (html_editor!=0)
	{
		CKEDITOR.remove(html_editor);
...
//------------------------------------------------------------------------------
function typed_objects_html_edit_save(id)
{
	if (html_editor==0) return;
	html=html_editor.getData();
	res=execQuery(typed_objects_widget_ajax_manager_url, {section : "typed_objectsHtmlPropEncode", html: html});
	$('#propt_'+id).html(html_editor.getData());
	$('#prop_'+id).val(res);
	typed_objects_html_edit_cancel();
}
//------------------------------------------------------------------------------
function typed_objects_prop_edit_dir_add(dir_id, prop_id)
{
	admin_info_show('Добавить значение', "<img src='/admin/images/"+loading_icon+"'>", 200);
	res=execQuery(typed_objects_widget_ajax_manager_url, {section : "typed_objectsDirAddGetHTML", prop_id: prop_id, dir_id: dir_id});
	admin_info_change('', res);
}
//------------------------------------------------------------------------------
var type_dobjects_attachment_prop_id=0;
function typed_objects_prop_edit_attachment_edit(id)
{
    type_dobjects_attachment_prop_id=id;
	attachmet=$("#prop_"+id).val().split('|');
	common_attachment_edit(attachmet[0], attachmet[1], attachmet[2], 'typed_objects_prop_edit_attachment_edit_save');
}
// -----------------------------------------------------------------------------
function typed_objects_prop_edit_attachment_edit_save(att_id, name, file)
{
	id=type_dobjects_attachment_prop_id;
	$("#propf_"+id).val(file);
	$("#propn_"+id).val(name);
	$("#propt_"+id).html(name+" ("+file+")");
	$('#prop_'+id).val(att_id+'|'+file+'|'+name);
}
// -----------------------------------------------------------------------------
function typed_objects_prop_edit_attachment_delete(id)
{
	attachmet=$("#prop_"+id).val().split('|');
	$("#propf_"+id).val('');
	$("#propn_"+id).val('');
	$('#prop_'+id).val(attachmet[0]+'||');
	$("#propt_"+id).html("нажмите для загрузки документа");
//	if (attachmet[0]!='')
//	{
//		$("#propt_"+id).html("удаление");
//		common_attachement_delete(attachmet_id)
//		$("#propt_"+id).html("нажмите для загрузки документа");
//	}
}
// -----------------------------------------------------------------------------
