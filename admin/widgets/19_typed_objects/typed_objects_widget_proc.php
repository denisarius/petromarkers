...
stop;
			...
			case 'file':	// Attached file
				...
				if ($att['name']=='') $att_name='нажмите для загрузки документа';
	...
	foreach($details as $k=>$v)
	{
		...
		if ($v!='' && $type!='')
		{
			switch($type)
			{
				...
				case 'file':
					$att=explode('|', $v);
					$att[1]=iconv ($html_charset, 'utf-8', $att[1]);    // возвращаем имя файла в UTF-8
...
function typed_objects_object_delete($id)
{
	...
	if ($obj_type!==false)
	{
		...
		foreach($object_description['details'] as $obj_prop)
		{
// обрабока всех параметров типа "структурированный тест"
// удаляем записи в таблице $_cms_text_parts и связанные изображения
			if($obj_prop['type']=='st')
			{
				$prop_id=get_data('id', $_cms_objects_details, "node='$id' and typeId='{$obj_prop['id']}'");
...
// обрабока всех параметров типа file
