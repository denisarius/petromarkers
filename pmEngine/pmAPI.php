...
function pmImportVarsList($__names__, $mysql_safe=false)
{
	...
	if (count($__names_list__)==1) $vars=current($vars);
	return $vars;
}
//------------------------------------------------------------------------------
...
{
	$text=strip_tags($text, '<p><li>');
	$text=strtr($text, array('<li>'=>'', '</li>'=>'<br>', '<p>'=>'', '</p>'=>'<br>'));
	if (strlen($text)<$len) return $text;
	while($text[$len]!=' ' && $text[$len]!=',' && $text[$len]!='.' && $len<strlen($text)-1)
		$len++;
